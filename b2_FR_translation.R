...
WD <- dirname(dirname(normalizePath(sys.frame(1)$ofile)))
setwd(WD)

# This script is only needed for the French data
FR_df <- read_xlsx(
  file.path("data", "tables", "crop_names", "FR_FR_crop_names_w_EuroCrops_class.xlsx")
)
cultures <- read.csv2(
  file.path("data", "vector", "IACS", "FR", "REF_CULTURES_2021.csv")
)
...
# save file
write_xlsx(FR_FR_crop_names_w_translation_without_abbreviations, file.path("data", "vector", "IACS", "FR", "FR_FR_crop_names_w_translation_without_abbreviations.xlsx")
